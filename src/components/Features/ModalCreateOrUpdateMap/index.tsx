...
async function createMap(data: MapFormValues, imageFile: File): Promise<ValorantMap> {
...
export const ModalCreateOrUpdateMap = ({ map, onClose, onSuccess }: MapModalProps) => {
  ...
  const onSubmit = async (data: MapFormValues) => {
    ...
    try {
      let saved: ValorantMap;

      if (isEditing) {
        saved = await updateMap(map!.id, data, imageFile ?? undefined);
      } else {
        if (!imageFile) throw new Error('Imagem obrigatória');
        saved = await createMap(data, imageFile);
      }
    ...
    <Dialog.Root open onOpenChange={(open) => !open && onClose()}>
      ...
      <Dialog.Content
        ...
        <div className="flex items-center justify-between p-6 pb-0">
          <Dialog.Title className="text-2xl font-bold">{isEditing ? 'Editar Mapa' : 'Criar Mapa'}</Dialog.Title>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="p-6 space-y-5" noValidate>
          ...
          {submitStatus === 'success' && (
            <div className="flex items-center gap-2 p-3 bg-green-50 border border-green-200 rounded-lg text-green-700 text-sm">
              ...
              {isEditing ? 'Mapa atualizado com sucesso!' : 'Mapa criado com sucesso!'}
            </div>
          )}
